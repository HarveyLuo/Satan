...
class mysqlPDO {
	...
	public function __construct($options){
		try{
			...
			switch($type){
				case 'mysql':case 'pgsql':$this->pdo=new PDO($type.':host='.$this->server.';'.$port.'dbname='.$this->database_name,$this->username,$this->password,$this->option);
				break;
				case 'mssql':case 'sybase':$this->pdo=new PDO($type.':host='.$this->server.';'.$port.'dbname='.$this->database_name.','.$this->username.','.$this->password,$this->option);
				break;
	...
	protected function data_implode($data,$conjunctor,$outer_conjunctor=null){
			...
			foreach($data as $key=>$value){
				if(($key=='AND'||$key=='OR')&&is_array($value)){
					...
					if(isset($match[3])){if($match[3]==''||$match[3]=='!'){
					...
					switch(gettype($value)){
						...
						case 'array':$wheres[]=$column.' IN ('.$this->array_quote($value).')';
							break;
						case 'integer':$wheres[]=$column.' = '.$value;
							break;
	...
	public function where_clause($where){
		...
		if(is_array($where)){
			...
			if(isset($where['GROUP'])){
				$where_clause.=' GROUP BY '.$this->column_quote($where['GROUP']);}if(isset($where['ORDER'])){
	...
	public function insert($table,$data){
		...
		foreach($data as $key=>$value){
			switch(gettype($value)){
				...
				break;
				case 'array':$values[]=$this->quote(serialize($value));
				break;
				case 'integer':case 'string':$values[]=$this->quote($value);
				break;
			}
		}
			...
			if(is_array($value)){
				...
				if(isset($match[3])){
					...
					switch(gettype($value)){
						...
						break;
						case 'array':$fields[]=$key.' = '.$this->quote(serialize($value));
						break;
						case 'integer':case 'string':$fields[]=$key.' = '.$this->quote($value);
						break;
						}
					}
				}
